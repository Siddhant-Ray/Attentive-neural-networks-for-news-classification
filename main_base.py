...
from helper import classDataset
...
# Plot the number of entries per category and the average length per category
plot_preliminary_dataset = data_set.data_plotter()
...
### Saved old model on original dataset
path = os.getcwd()
...
# Generate a confusion matrix based on the actual values vs predicted values
...
### Flag to generate t-SNE

generate_tSNE = False

if generate_tSNE == False:
    exit(0)

### As this is the base model, we now re-run the model for 2 epochs and extract the pre-classifier layer
...
full_dataset = classDataset(all_encodings, all_labels)
full_dataset_loader = DataLoader(full_dataset, batch_size=16, shuffle=True)

### Run the model on the whole dataset without training i.e. in eval mode 
pre_outputs_states = []
all_labels = []
